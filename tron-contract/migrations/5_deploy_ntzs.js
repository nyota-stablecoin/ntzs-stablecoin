const Ntzs = artifacts.require("Ntzs");
...
module.exports = async function (deployer, network, accounts) {
  ...
  // Deploy Ntzs logic contract
  await deployer.deploy(Ntzs);
  const NtzsLogic = await Ntzs.deployed();
  console.log("Ntzs logic contract deployed to:", NtzsLogic.address);
  ...
  console.log("Ntzs proxy deployed to:", proxy.address);
  
  try {
    // Create an instance of the Ntzs contract at the proxy address for verification
    const NtzsProxy = await Ntzs.at(proxy.address);
    console.log("Proxy initialized with admin operations at:", await NtzsProxy.adminOperationsContract());
    console.log("Proxy initialized with trusted forwarder at:", await NtzsProxy.trustedForwarderContract());
  } catch (error) {
